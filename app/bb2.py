...
def create_container_root(image_name, image_dir, container_id, container_dir): #Create a directory for the container rootfs
    ...
    # print(image_path)
    container_root = _get_container_path(container_id, container_dir, 'rootfs') #Get the container path
    assert os.path.exists(image_path), f"unable to locate image {image_name}" #Check if the image path exist
    
    #Create a directory if it does not exist
    if not os.path.exists(container_root): #Create the path if it doesnot exist
        ...
        with tarfile.open(image_path) as tar_file:
            ...
            tar_file.extractall(container_root, members=filtereed_members) #Extract all to the file path we created

    return container_root #Return a path to the rootfs
...
def contain(command, image_name, image_dir, container_id, container_dir):
    new_root = create_container_root(image_name, image_dir, container_id, container_dir) #Create a new container rootfs
    print(f'Created a new rootfs for container: {container_id} at {new_root}') #To show that container was created
    ...
    for i, dev in enumerate(['stdin', 'stdout', 'stderr']): #iterate through the list, assign each value an index
        os.symlink(f'/proc/self/fd/{i}', os.path.join(new_root, 'dev', dev)) #creates a symbolic link pointing from /proc/self/fd/i (which is typically the stdin, stdout and stderr) to the file inside the dev directory of the container's new root.

    os.chroot(new_root) #Jail the process
    os.chdir('/') #Change to the root directory
    os.execvp(command[0], command) #Execute the command in our new container
